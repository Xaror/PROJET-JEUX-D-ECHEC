...
static UINT16 historyValueDummy[HISTORY_SIZE];
static INT16 positionalGainDummy[HISTORY_SIZE];
...
static char *getGameText(PGNFile * pgnfile, int number)
{
   ...
   numBytesRead = fread(buffer, 1, length, pgnfile->file);
   buffer[length] = '\0';
   trim(buffer);
...
{
   Gamemove *new = (Gamemove *) malloc(sizeof(Gamemove));
...
static void freeGamemove(Gamemove * move)
{
   if (move->nextMove != 0)
   {
      freeGamemove(move->nextMove);
   }

   if (move->alternativeMove != 0)
   {
      freeGamemove(move->alternativeMove);
   }
...
int appendMove(PGNGame * game, const Move move)
{
   ...
   {
      Gamemove *newMove =
         getGamemove(&variation.singlePosition, game->lastMove);
                               ...
                               size_t * offset, const Position * position)
{
          ...
          (currentChar = pgnMoveText[(*offset)++]) != '\0')
   {
      ...
      if (strchr("KQRBNOabcdefgh", currentChar) != 0)
      {
         ...
         {
            Gamemove *newMove =
               getGamemove(&variation.singlePosition, lastMove);
                                  ...
                                  const Position * position)
{
   ...
   return parseMoveText(pgnMoveText, &offset, position);
}
...
static char *generateMoveSection(Gamemove * gamemove, const char *result)
{
   ...
   variation.historyValue = &(historyValueDummy[0]);
   variation.positionalGain = &(positionalGainDummy[0]);
...
void initializeVariationFromGame(Variation * variation, PGNGame * game)
{
   if (game->lastMove != 0)
   {
      ...
      {
         variation->positionHistory[i--] = currentMove->position.hashValue;
                                 ...
                                 bool copyPv)
{
   ...
   if (copyPv != FALSE)
   {
      ...
      {
         Move move = (Move) variation->pv.move[i];
