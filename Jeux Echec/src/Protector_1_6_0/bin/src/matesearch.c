                      ...
                      PrincipalVariation * pv)
{
   ...
   for (i = POSITION_HISTORY_OFFSET + variation->ply - 4;
        i >= historyLimit; i -= 2)
   {
      if (variation->singlePosition.hashValue ==
          variation->positionHistory[i])
      {
         return 0;
      }
   }
 ...
 finish:
   ...
   setDatedEntry(variation->hashtable, position->hashValue,
                 ...
                 packedMove(bestMove), hashentryFlag, 0, 0);
                           ...
                           Movelist * solutions)
{
   ...
   while ((currentMove = getNextMove(&movelist)) != NO_MOVE)
   {
      ...
      if (value >= best)
      {
         best = variation->pv.score = value;
         setMoveValue(&currentMove, value);
         appendMoveToPv(&pv, &variation->pv, currentMove);
...
void searchForMate(Variation * variation, Movelist * foundSolutions,
                   int numMoves)
{
   int iteration, numSolutions = 0, i;
   ...
   for (iteration = 1; iteration <= numMoves; iteration++)
   {
      variation->nominalDepth = 2 * iteration - 1;

      searchBaseMoves(variation, -VALUE_MATED - 2 * iteration,
                      -VALUE_MATED - 2 * iteration + 1,
                      variation->nominalDepth, foundSolutions);
   }
