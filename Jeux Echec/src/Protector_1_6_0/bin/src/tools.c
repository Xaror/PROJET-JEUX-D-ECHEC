...
unsigned long getTimestamp()
{
   ...
   int count = 0;
   struct _timeb t_current;

   while (count++ < 5)
   {
      if (_ftime_s(&t_current) == 0)
      {
         return 1000 * (unsigned long) t_current.time +
            (unsigned long) t_current.millitm;
      }
   }
...
String *appendToString(String * string, const char *fmt, ...)
{
   ...
   va_start(args, fmt);
   numBytesWritten = vsprintf(buffer, fmt, args);
   va_end(args);
