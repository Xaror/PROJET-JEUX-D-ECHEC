...
#if defined TARGET_WINDOWS && defined SUPPORT_SMP
#include <Windows.h>

volatile long tbAccessCount = 0;
#endif

#if defined TARGET_LINUX && defined SUPPORT_SMP
#include <pthread.h>
...
int probeTablebase(const Position * position)
{
   ...
   while (InterlockedIncrementAcquire(&tbAccessCount) > 1)
   {
      InterlockedDecrement(&tbAccessCount);
   }
#endif
